...
public final class testSimValueConstructorMappableGrp {
    ...
    public double[] loadSimvalueFromDB(final testPair pair, final double[] weight,
        ...
        String concept1 = source_ontology.getElement(pair.getSource()).getLocalName();
        String concept2 = target_ontology.getElement(pair.getTarget()).getLocalName();
        ...
        for(Integer i : source_content)
        {
            ...
            for(Integer j : target_content)
            {
                ...
                conceptPairs = concept1ID.concat(AlgoConstants.SEPERATOR).concat
                        (concept2ID);
                if(mappableConcepts.contains(conceptPairs) && source_ontology.getElement(source_element_uri).getAlignElement() == null
                        || target_ontology.getElement(target_element_uri).getAlignElement() == null){
    ...
    private void perform(int matcher, Matcher[] matcherList, double[] weight) {
        ...
        for(String concepts : mappableConcepts) {
            ...
            // Quering ontoManager with conceptID.
            concept1 = source_ontology.getElementURI(conceptIDs[0]);
            concept2 = target_ontology.getElementURI(conceptIDs[1]);
            ...
            // Getting pretty name
            concept1Name = source_ontology.getElement(pair.getSource()).getPrettyName();
            concept2Name = target_ontology.getElement(pair.getTarget()).getPrettyName();
            //concept ID
            concept1ID = source_ontology.getElement(pair.getSource()).getLocalName();
            concept2ID = target_ontology.getElement(pair.getTarget()).getLocalName();
            simValueFound=false; 
            isPairFound=false;                
            /**
            ...
            if (!simValueFound) {
                ...
                if(source_ontology.getElement(pair.getSource()).isMClass() &&
                        target_ontology.getElement(pair.getTarget()).isMClass()){
